...
module.exports.eventAddOne = function (req, res) {
    ...
    if (req.body && req.body.title && req.body.date) {
        ...
        newEvent.urlPicture = req.body.urlPicture;
        newEvent.date = req.body.date;
        newEvent.place = req.body.place;
        Event.create(newEvent, function (err, eventAdded) {
            if (err) {
        ...
        if (err) {
            ...
            if (event) {
                if (req.body && req.body.title && req.body.date) {
                    ...
                    Event.findByIdAndUpdate(eventId, newEvent).exec(function (err, eventUpdated) {
                        if (err) {
                            console.log("error adding one event", err);
                            response.status = 400;
                            response.message = err;
